...
> Exception: the Vitest configs keep their `@workspace/core` alias on purpose — see [The exception that stays](#the-exception-that-stays).
...
**The `@workspace/core/*` tsconfig mapping**, from `apps/web`. All imports across the repo use subpaths the `exports` map already covers, so the alias added nothing but a second answer to the same question. The web app resolves `bundler`, which reads `exports`.

**`"*": ["./*"]`**, from the app tsconfig. It let any bare specifier resolve against the app root for `tsc` alone. No bundler does this, so a genuinely missing dependency would typecheck and then fail to build.
...
`packages/core/src/libs/i18n/my-translations.d.ts` was imported by `init.ts` as `.../my-translations.d`, and augmented by the app provider as `.../my-translations`:
...
The two numbers say different things, and the split is deliberate: **`engines.node` (`>=26.0.0`) is a support policy — what will run. `.node-version` is a pin — what we check.** `engines` is on the root and on `apps/web`, that being the sole workspace with a Node runtime; `packages/core` is source only. Asserting a Node floor for core would state a constraint its artifacts do not have. Bun does not enforce `engines` on install in any case — the floor is enforced by CI reading `.node-version`, not by the package manager. `packageManager: bun@1.3.14` pins Bun, and the Dockerfile's `oven/bun` stages match it.
...
- **Adding `engines.node` to every workspace** — rejected; it would assert a runtime constraint that `core` artifacts do not have, and Bun does not enforce it anyway.
...
`bun run typecheck` and `bun run test:unit` (51 files, 343 tests) pass. `bun web build` fails in the local sandbox — Turbopack's PostCSS worker cannot spawn a process or bind a port — **identically before and after this change**, confirmed by building the stashed tree; it is unrelated and unaffected.
