...
**The `@workspace/core/*` tsconfig mapping**, from `apps/spa` and `apps/web`, and **the matching Vite alias** in `apps/spa/vite.config.ts`. All imports across the repo use subpaths the `exports` map already covers, so the alias added nothing but a second answer to the same question. Both apps resolve `bundler`, which reads `exports`.

**`"*": ["./*"]`**, from both app tsconfigs. It let any bare specifier resolve against the app root for `tsc` alone. No bundler does this, so a genuinely missing dependency would typecheck and then fail to build. It was covering nothing: of 154 bare specifiers across the two apps, none resolved through it.
...
- **Adding `engines.node` to every workspace** — rejected; it would assert a runtime constraint that `spa` and `core` artifacts do not have, and Bun does not enforce it anyway.
